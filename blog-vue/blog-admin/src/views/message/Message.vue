  ...
  <el-card class="main-card">
    ...
    <div class="review-menu">
      ...
      <span
        ...
        :class="isAudit == null ? 'active-review' : 'review'"
      >
        全部
      </span>
      <span
        @click="changeReview(1)"
        :class="isAudit == 1 ? 'active-review' : 'review'"
      >
        正常
      </span>
      <span
        @click="changeReview(0)"
        :class="isAudit == 0 ? 'active-review' : 'review'"
      >
        审核中
      </span>
    </div>
    ...
    >
      ...
      />
      <!-- 状态 -->
      <el-table-column prop="isAudit" label="状态" width="80" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.isAudit == 0" type="warning">审核中</el-tag>
          <el-tag v-if="scope.row.isAudit == 1" type="success">正常</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="gmtCreate"
...
export default {
  ...
  methods: {
    ...
    deleteMessage(id) {
      ...
      this.axios.delete("/api/admin/message", param).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
    ...
    updateMessageReview(id) {
      ...
      param.isAudit = 1;
      this.axios.put("/api/admin/message/audit", param).then(({ data }) => {
        if (data.status) {
          this.$notify.success({
            title: "成功",
            message: data.message,
