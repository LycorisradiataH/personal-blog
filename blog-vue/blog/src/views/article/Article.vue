        ...
        <div class="article-info">
          <div class="first-line">
            ...
            <span>
              ...
              发表于 {{ article.gmtCreate | date }}
            </span>
            <span class="separator">|</span>
            <!-- 更新时间 -->
            <span>
              <i class="iconfont icon-update-date"></i>
              更新于
              <template v-if="article.gmtModified">
                {{ article.gmtModified | date }}
              </template>
              <template v-else>
                {{ article.gmtCreate | date }}
              </template>
            </span>
            <span class="separator">|</span>
            <!-- 文章分类 -->
              ...
              <div class="animated fadeInDown reward-main">
                <ul class="reward-all">
                  <li class="reward-item">
                    <img
                      class="reward-img"
                      :src="blogInfo.websiteConfig.weChatQRCode"
                    />
          ...
          >
            ...
            <div class="recommend-list">
              <div
                ...
                v-for="item in article.recommendArticleList"
                :key="item.id"
              >
        ...
        <div style="position: sticky;top: 20px">
          ...
          <v-card class="right-container">
            <div class="right-title">
              <i class="iconfont icon-nav" style="font-size: 16.8px"></i>
          ...
          <v-card class="right-container" style="margin-top: 20px">
            <div class="right-title">
              <i
                class="iconfont icon-update-date"
            ...
            <div class="article-list">
              <div
                class="article-item"
                v-for="item in article.newestArticleList"
                :key="item.id"
              >
...
export default {
  ...
  methods: {
    getArticle () {
      ...
      getArticleById(this.$route.path).then(({ data }) => {
        ...
        this.$nextTick(() => {
          ...
          tocbot.init({
            tocSelector: '#toc', // 把目录添加元素位置，支持选择器
    ...
    listComment () {
      ...
      const current = 1
      listComment(articleId, current).then(({ data }) => {
    ...
    like () {
      ...
      likeArticle(this.article.id).then(({ data }) => {
        if (data.status) {
          ...
          this.$store.commit('articleLikeSet', this.article.id)
        }
      })
    },
